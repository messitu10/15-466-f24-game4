...
struct Character {
    ...
    static Character Load(hb_codepoint_t code_point, FT_Face ft_face)
    {
        //https://learnopengl.com/In-Practice/Text-Rendering
        if (FT_Load_Glyph(ft_face, code_point, FT_LOAD_RENDER))
            throw std::runtime_error("Failed to load glyph: " + std::to_string(code_point));
        GLuint tex;
        glGenTextures(1, &tex);
...
struct Text {
    const std::string text_file = "OpenSans-SemiBold.ttf";
    float font_size = 64.f;
    float font_scale = 64.f;

    FT_Library ftlibrary;
    FT_Face typeface;
    hb_buffer_t* hb_buffer;
    hb_font_t* hb_typeface;
    ...
    void init()
    {
        ...
        {
            set_font_size(font_size, font_scale, true);
    ...
    void showScore(uint32_t score){
        update_buffer("Your Final Listening Score is: " + std::to_string(score) + "/5");
    }
